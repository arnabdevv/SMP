...
const ClassManagement = () => {
  const { t } = useLanguage();
  const { classes, batches, addClass, addBatch, getBatchesByClass } = useData();
  ...
  const handleBatchSubmit = (e) => {
    e.preventDefault();
    const selectedClassObj = classes.find(cls => cls.id === batchFormData.classId);
    if (selectedClassObj) {
      addBatch({
        ...batchFormData,
  ...
  const classColumns = [
    ...
    { 
      ...
      render: (_, row) => getBatchesByClass(row.id).length
    },
    { 
      key: 'createdAt', 
      label: t('common.date'),
  ...
  const batchColumns = [
    ...
    { key: 'studentCount', label: t('common.students') },
    { 
      key: 'teacherIds', 
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="p-4 border-b border-gray-200 flex justify-between items-center">
          <h2 className="text-lg font-medium text-gray-900">{t('nav.classes')}</h2>
          <button
        ...
        <DataTable
          ...
          data={classes}
          onView={(row) => setShowBatches(row.id)}
        />
      </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="p-4 border-b border-gray-200 flex justify-between items-center">
            <h2 className="text-lg font-medium text-gray-900">
              {t('common.batch')}s - {classes.find(cls => cls.id === showBatches)?.name}
            </h2>
            <button
          ...
          <DataTable
            ...
            data={getBatchesByClass(showBatches)}
          />
        </div>
      )}
    </div>
